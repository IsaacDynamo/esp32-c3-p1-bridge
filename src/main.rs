...
use esp_buffered_serial::{BufferedSerial, Config, DataBits, Inverse, Parity, Pins, StopBits};
...
fn is_starting(wifi_interface: &esp_wifi::wifi_interface::Wifi) -> bool {
    matches!(
        ...
        Status(ClientStatus::Starting, _)
    )
}
...
fn is_started(wifi_interface: &esp_wifi::wifi_interface::Wifi) -> bool {
    matches!(
        wifi_interface.get_status(),
        Status(ClientStatus::Started(_), _)
    )
}
...
fn main() -> ! {
    ...
    let mut serial1 = SERIAL
        .init(peripherals.UART1, config, pins, &clocks)
        .unwrap();
    ...
    loop {
        ...
        let tcp_socket = wifi_interface
            .network_interface()
            .get_socket::<TcpSocket>(tcp_handle);
            ...
            tcp_socket
                .recv(|bytes| {
                    ...
                    let n = bytes
                        .iter()
                        .copied()
                        .map_while(|byte| match serial1.write(byte) {
                            Ok(_) => Some(()),
                            Err(_) => None,
                        })
                        .count();
            ...
            if uplink_state == UplinkState::Empty {
                ...
                tcp_socket
                    .send(|bytes| {
                        ...
                        let n = bytes
                            .iter_mut()
                            .map_while(|byte| match serial1.read() {
                                Ok(c) => {
                                    *byte = c;
                                    Some(())
                                }
